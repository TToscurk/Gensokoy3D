...
export const entries = {
  ...
  // 從霧之湖翻埡口上來。**落點必須離 mountain_to_lake 的觸發區夠遠** ——
  // 原本在 (18,-112)，距那個 r=12 的觸發區只有 8.9m（在裡面），
  // 一上山就被立刻送回湖邊：妖怪之山用走的永遠到不了，
  // 連帶天狗聚落／守矢／天界整支都進不去。
  //
  // 挪到山麓台階上（餘裕 54m）。這裡海拔 45m，和原落點同一個高度帶 ——
  // 從湖邊走上來不該一步瞬移到半山腰，雖然山上的坡比較平。
  // （這座山是火山錐，整片都是坡，舊落點自己的坡差也有 21.8m。）
  from_lake: { x: 30, z: -38, facing: Math.PI * -0.1 },
  // 從天狗聚落下來 —— 站在站體西側的坡道上
