...
export const meta = {
  ...
  sizeX, sizeZ,           // 選用：狹長的圖（參道 200×600）
  spawn: { x: 0, y: 0, z: 30 },
  fog: 0x9fb4cc,
  accent: 0xc2382f,
  sky: 'day',             // 天空預設，異界可換
  ...
  // 這張圖的高度場活在哪個座標空間 —— 決定遠景裙襬怎麼做。
  //   'world'（預設）從舊世界的高度場切出來的圖。局部座標 + ORIGIN
  //           就是世界座標，所以裙襬只要繼續往外取樣同一個高度場，
  //           遠景就是地理上真的那座山，免費得到正確的天際線。
  //   'local'  自己造地形的圖（參道、室內、異界）。世界座標對它沒有意義，
  //           **必須自己提供邊緣過渡帶**（見下）。
  heightSpace: 'world',
  origin: { x, z },       // heightSpace: 'world' 時必填：這張圖的中心在世界座標的位置
};
...
| `style` | 表現 | 用在哪 |
|---|---|---|
| `'walk'` | 短暫淡出淡入 | 兩張圖直接相鄰、走過去就到 |
| `'gate'` | 黑幕 + 進度條 | 結界之外的遠方（白玉樓、天界、彼岸） |
| `'vehicle'` | 上車 → 運鏡 → 過場 → 在對岸的站下車 | 索道、渡船之類的交通工具 |

**`'vehicle'` 為什麼必須是 portal 的一種，而不是「會動的物件」**

妖怪之山的索道兩端在不同的場景裡（山麓站 ↔ 山頂守矢），中間那段纜線
**沒有任何一張圖擁有它**。如果照「單張圖內部的動畫」去做，結果會是兩張圖
各自有一台纜車在跑、纜線在圖邊被切斷 —— 看起來會動，實際上是壞的。

正確的模型是：**搭乘＝觸發切圖**。

```js
{
  id: 'ropeway_foot_to_top',
  to: 'moriya',
  entry: 'from_ropeway',
  trigger: { x, z, r },        // 月台範圍
  label: '搭乘索道',
  style: 'vehicle',
  vehicle: {
    board: { x, y, z },        // 車廂在這張圖的停靠位置
    dwell: 1.2,                // 上車後鏡頭停留幾秒再淡出
    camera: 'follow',          // 運鏡：跟著車廂爬升
  },
}
```

**纜線各圖只畫自己那半段**，往圖外延伸、消失在霧裡（裙襬與霧本來就會處理
遠處，不必真的接起來）。車廂也是各圖自己的道具，不是同一台在兩張圖之間移動。

### 遠景裙襬與 `heightSpace`

小圖只有 400～600 公尺，霧的遠端卻在 900 公尺 —— 地形會在霧遮住之前就被切出一條硬邊，外面是空的。所以**每張圖都要有遠景裙襬**：自己的地形之外再鋪一圈低解析度的背景地形，一路延伸到霧之外。

裙襬怎麼取樣，由 `heightSpace` 決定：

| `heightSpace` | 裙襬做法 | 適用 |
|---|---|---|
| `'world'` | 繼續往外取樣舊世界的高度場（`terrainHeight`） | 從舊世界切出來的圖 |
| `'local'` | 由地圖自己提供 `skirtHeightAt(x, z)`，負責從自己的地形**平滑過渡**到遠景 | 自己造地形的圖 |

`'world'` 的圖不必寫任何東西 —— 局部座標加上 `origin` 就是世界座標，遠景自動是地理上真的那片地貌。這也代表**舊世界的佈局沒有白做**，它變成所有小圖免費的天際線。

`'local'` 的圖（參道、之後的室內與異界）**必須**提供過渡，否則裙襬會和自己的地形對不上，接縫處出現斷崖。這是契約的一部分，不是個案處理 —— 白玉樓、天界、彼岸幾乎一定都是 `'local'`，不先訂好就會各自長出各自的臨時解法。
...
> **階段 2 完成紀錄（2026-07-27）** — tag `stage2-green`
>
> `shrine`（420×420）與 `sando`（200×600）互通，`style: 'walk'`。
> 驗收全過：`test_portal` 14/14、`test_scene_churn` 5/5（切 30 次
> geometries 82→84、textures 持平 42、heap +0.6MB）。
> draw call：shrine 39 / sando 40（門檻 200），回 legacy_open 86 ＝ 開機值。
>
> **`sando` 是新造的地景，不是從舊世界切出來的。** 那一帶實測是側傾山坡
> （橫向 100m 落差 57m），挖出來會走得很怪。所以它有自己的高度場。
> 這是階段 3 要沿用的判準：**能切就切，切出來不好走就自己造。**
>
> 這階段修掉的三個錯（都記在 commit 訊息裡）：還原持久層的判斷放錯端、
> NPC 距離剔除的邊緣觸發栓被繞過（多 264 draw call）、sando 燈籠沒合併。
>
> **階段 3 的待辦（這階段暴露出來的）**：
> - `clearings`（除草區）目前是全域清單、記世界座標。分圖是局部座標，
>   寫進去會污染 legacy_open。要改成每張圖各自一份 —— 在那之前，
>   新圖的草地不會讓開石板路。
> - NPC 目前是「整批顯示／隱藏」（§4 #8 的過渡做法），
>   要改成每張圖只生成 `meta.npcs` 列的那幾位。
> - `ui/mapview.js` 仍是舊世界的俯視底圖（§4 #4）。

> **階段 3 進度（2026-07-28 完成，18／18）**
>
> 已全部遷完：人間之里、魔法之森、霧之湖、紅魔館、迷途竹林、命蓮寺、
> 香霖堂、無緣塚、太陽花田、無名之丘、妖怪之山、天狗聚落、守矢神社、
> 白玉樓、天界、彼岸（連同首批的博麗神社、參道共 18 張）。
> 異界三張用 `style: 'gate'`；索道是 `style: 'vehicle'` portal（§1），
> 山麓站／山上站各畫半段纜線。tag `stage3-green`。
> 早期 7 張時的連線圖與效能表：
> ```
...
> | 紅魔館 | 460 | 37 | 69K | 5 |
> | 迷途竹林 | 600 | 39 | 98K | 4 |
> | legacy_open | 2400 | 88 | 547K | 34 |
>
> **遷移一張圖的固定流程**（照著做，五張都一樣）：
> 1. `buildStructureSet([id])` → `mergeStaticByMaterial` → 平移回原點
> 2. 碰撞盒／燈籠／除草區／室內互動點全部 `toLocal`
> 3. 植被下種前 `setClearings(st.clearings)`（世界座標那份），建完還原
> 4. `npcs` 只列 roster 裡 `region` 等於這張圖的角色
> 5. portal 成對定義，entry 與回程 trigger 至少隔 30m（不然會來回彈跳）
>
> **實測後使用者的回饋**：過場頻率可接受（最短一段疾走 21 秒）。
> 但**有些建築會擋到路** —— 舊世界就有的問題，遷移後仍在，
> 目前判定為可接受，要處理的話是調 `structures.js` 的碰撞盒而不是分圖。
>
> **踩到的坑（後續遷移會再遇到）**：
> - 沒有建築也沒有互動點的圖（霧之湖）會被 `applyEnv` 誤判成
>   legacy_open，把舊世界的持久層裝回來。地圖用 `ownsContent: true` 明講。
> - `test_scene_churn` 的基準取樣點必須跟後面的取樣停在**同一張圖**上，
>   否則「圖不一樣」會被誤讀成「洩漏」。取樣間隔也要是循環長度的整數倍。
> - **`buildVegetation` 裡「固定位置的地景」（竹林、太陽花田、鈴蘭）
>   不受下種範圍影響**，每張小圖都把整片建一次，而 instanced 網格是
>   `frustumCulled = false` 的 —— 看不見也照畫。已改成依範圍取捨。
> - **`staticLights` 原本沒有接進分圖**（室內常亮吊燈）。不處理的話
>   紅魔館的吊燈會亮在別張圖的空中，而遷過來的館裡反而是黑的。
> - **會動的物件要跟著搬**：紅魔館的時鐘指針原本由主迴圈每幀轉，
>   遷進地圖後歸該圖的 `update()` 管，否則館裡的鐘會停住。
>   **注意索道不屬於這一類** —— 它跨兩張圖，是 portal 的一種形態
>   （`style: 'vehicle'`，見 §1），當成單張圖的動畫做會得到
>   「兩張圖各有一台纜車在跑、纜線在圖邊被切斷」的結果。
