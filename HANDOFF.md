...
`legacy_open` 是孤島（沒有 portal），**2026-07-28 起開機直接進分圖（shrine）**，舊世界退居備援——
從設定面板的傳送清單仍可進去，建築與住民都在，但不再維護。
...
- **地圖系統已分圖化（2026-07-28）**：大地圖（M）改成連線圖 —— 19 節點（18 分圖＋舊世界
  時光機）按地區世界座標布局、portal 連線去重繪製、點節點走 `manager.load`（異界三張用
  gate 過場）、現行節點紅圈高亮、任務金點落在目標所在節點；小地圖底圖改成現行圖的
  局部俯視烘焙（224²，切圖時 `mapView.onMapChanged()` 重烘，`legacy_open` 才用舊的
  448² 世界烘焙），任務金點只顯示同圖目標。參道沒有 REGIONS 地區圓，節點位置在
  `mapview.js` 的 `NO_REGION` 表手給。驗收：`test_bigmap.mjs`（12 項）、`test_map.mjs`
  已改分圖版
...
分圖版：**大地圖（M）是連線圖** —— 19 個地圖節點按地區世界座標布局，portal 連線，
點節點走 `manager.load` 傳送（異界三張 gate 過場），現行節點高亮、任務金點落在
目標所在節點。**小地圖**底圖是現行圖的局部俯視烘焙（`map.heightAt`，224²，
切圖時 `onMapChanged()` 重烘）；只有 `legacy_open` 用舊的 448² 世界烘焙（lazy）。
坡向陰影從高度格差分算，**不要每像素多叫一次高度函數**。
